...
describe('Provisioning CLI (Integrated)', () => {
    ...
    beforeEach(() => {
        vi.clearAllMocks();
        vi.mocked(NestFactory.createApplicationContext).mockResolvedValue(mockApp as any);
    ...
    it('should fail when missing subdomain', async () => {
        await expect(main(['--email=test@test.com', '--password=Pass123!', '--store-name=TestStore']))
    ...
    it('should accept quiet flag and suppress logs', async () => {
        mockProvisioningService.provision.mockResolvedValue({ subdomain: 'test', durationMs: 0 });
        const logSpy = vi.spyOn(console, 'log').mockImplementation(() => { });

        await main(['--subdomain=test', '--email=t@t.com', '--password=p', '--store-name=s', '--quiet']);
