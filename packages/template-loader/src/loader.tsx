...
export class TemplateLoader {
     ...
     */
    async loadComponent<T = any>(templateName: string, componentPath: string = 'layout'): Promise<ComponentType<T>> {
        ...
        try {
            ...
            const module = await import(`@templates/${templateName}/src/app/${componentPath}`);
            const Component = module.default;

            if (!Component) {
                throw new Error(`Template component "${componentPath}" in "${templateName}" does not have a default export`);
            }
            ...
            console.error(`Failed to load template component: ${templateName}/${componentPath}`, error);
    ...
    bindTenantContext<T = any>(
        ...
        BoundComponent.displayName = `Bound(${Component.displayName || Component.name || 'Component'})`;
