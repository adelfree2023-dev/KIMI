...
export class TemplateSecurityValidator {
    ...
    async validateTemplate(templatePath: string): Promise<ValidationResult> {
        ...
        console.log(`  │  ${s2Result.passed ? '✅' : '❌'} Score: ${s2Result.score}/100 (${s2Duration}ms)`);
        ...
        console.log(`  │  ${s3Result.passed ? '✅' : '❌'} Score: ${s3Result.score}/100 (${s3Duration}ms)`);
        ...
        console.log(`  │  ${s7Result.passed ? '✅' : '❌'} Score: ${s7Result.score}/100 (${s7Duration}ms)`);
        ...
        const fatalCount = allViolations.filter(v => v.severity === 'FATAL').length;
        ...
        console.log(`\n  └─ Overall: ${passed ? '✅ PASSED' : '❌ FAILED'} | Score: ${overallScore}/100`);
        console.log(`     Violations: ${fatalCount} fatal, ${warningCount} warnings (${totalDuration}ms)\n`);
    ...
    generateSummary(result: ValidationResult): string {
        ...
        lines.push(`- **Violations:** ${result.summary.fatalViolations} fatal, ${result.summary.warningViolations} warnings`);
