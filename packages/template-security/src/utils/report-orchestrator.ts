...
async function orchestrate() {
    ...
    const scripts = [
        { name: 'S1: Env Config', path: 'tools/security-scripts/verify-env-config.ts', gate: 'S1' },
        { name: 'S3: Input Validation', path: 'tools/security-scripts/verify-input-validation.ts', gate: 'S3' },
        { name: 'S4: Audit Logs', path: 'tools/security-scripts/verify-audit-logs.ts', gate: 'S4' },
    ];
    ...
    try {
        ...
        const templateReport = JSON.parse(readFileSync('templates/fashion-boutique/.security-reports/validation-report.json', 'utf-8'));

        // Merge violations
        Object.values(templateReport.phases).forEach((phase: any) => {
            reports.protocols.violations.push(...phase.violations.map((v: any) => ({
                rule: phase.name,
        ...
        console.warn('  ⚠️  Modern Template Validator skipped or failed (template not found)');
    }
    ...
    try {
        ...
        execSync('bun src/scanners/pattern-scanner.ts --path=templates', { stdio: 'pipe' });
        const patternReport = JSON.parse(readFileSync('static-analysis-report.json', 'utf-8'));
    ...
    writeFileSync('s1-s9-report.json', JSON.stringify(reports.protocols, null, 2));
    writeFileSync('static-analysis-report.json', JSON.stringify(reports.static, null, 2));
