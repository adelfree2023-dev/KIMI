...
export class S7EncryptionChecker {
    async validate(templatePath: string): Promise<CheckResult> {
        ...
        const piiPatterns = ['email', 'phone', 'address', 'card', 'ssn', 'jwt', 'token', 'password'];

        for (const file of codeFiles) {
            const content = await readFile(file, 'utf-8');

            // 1. Check for PII in localStorage
            const localStorageMatches = content.matchAll(/localStorage\.setItem\(['"]([^'"]*)['"]/g);
            ...
            const consoleMatches = content.matchAll(/console\.(log|info|debug)\(.*?(email|phone|password|card)/gi);
            ...
            if (cardInputMatches && Array.from(cardInputMatches).length > 0) {
                const hasStripeElements = content.includes('CardElement') ||
