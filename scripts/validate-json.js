...
function validateJson(filePath) {
    try {
        ...
        if (posMatch) {
            ...
            const sanitizedDebug = raw.replace(/("(?:[^"\\]|\\.)*")|(?:\/\*[\s\S]*?\*\/|\/\/[^\n\r]*)/g, (m, s) => s || '');
            const sContext = sanitizedDebug.substring(Math.max(0, pos - 20), Math.min(sanitizedDebug.length, pos + 20));
            console.error(`Sanitized context: "...${sContext.replace(/\n/g, '\\n')}..."`);
        }
        return false;
    }
}
