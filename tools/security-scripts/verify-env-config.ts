...
function verifyEnvConsistency() {
    ...
    if (missingInSchema.length > 0) {
        console.error('❌ S1 VIOLATION: Keys in .env.example are missing from config schema:');
    ...
    if (missingInExample.length > 0) {
        ...
        console.error('❌ S1 VIOLATION: Keys in config schema are missing from .env.example:');
    ...
    console.log('✅ S1: Environment consistency verified (Schema vs .env.example)');
}
